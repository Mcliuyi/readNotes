...
​	使用事务日志，在存储引擎修改表数据时只需要修改拷贝内存，再把修改的行为记录在持久化日志中，避免每次将修改数据本身持久化到磁盘，事务日志使用的是追加的方式。即使在修改数据时服务崩溃，只要开启了事务日志就能恢复被修改的数据

#### 1.3.4 MySQL的事务

​		不是所有的存储引擎都支持事务，MySQL的主要支持事务的引擎为：**InnoDB**和**NDB Cluster**，还有一些第三方引擎

- **自动提交(AUTOCOMMIT)**

    Mysql**默认**是采用自动提交模式，只要**不显式**的指明使用事务，默认就是**一条SQL语句便是一个事务**。

  ​	通过`show variables like 'autocommit'`可以查看当前模式，**1为开启**，**0为禁用**，如果是禁用状态，所有的查询都是在一个事务中，必须显式的执行**COMMIT**提交或者**ROLLBAK**回滚，改事务结束，**自动开启下一个事务**。

  ​	除此之外，还有一些特殊的命令，是默认强制提交的，典型的就是**数据库定义语言(DDL: create, alter table...)**

  ​	通过以下命令可以设置事务的隔离性

  ```mysql
  set session transaction isolation level read committed; # 设置事务为读提交
  ```
...
| --------------- | ------------------------------------------------------------ |
| Name            | 表名                                                         |
| Engine          | 表的存储引擎，老版本中为Type                                 |
| Row_format      | 行的格式,包含Dynamic(可变), Fixed(固定), Compressed(压缩)    |
| Rows            | 行数,注: InnoDB是预估值                                      |
| Avg_row_length  | 平均每行长度                                                 |
| Data_length     | 表数据大小，字节为单位                                       |
| Max_data_length | 表数据最大容量，和存储引擎相关                               |
| Index_length    | 索引大小，字节为单位                                         |
| Data_free       | 对于MyISAM表，表示分配但目前没有使用的空间，包括之前删除的行，以及后续可以被INSERT利用的空间 |
| Auto_increment  | 自增长值的下一个值                                           |
| Create_time     | 创建时间                                                     |
| Update_time     | 数据最后修改时间                                             |
| Check_time      | 使用`CHECK TABLE`命令或者`myisamchk`工具最后一次检查表的时间 |
| Collation       | 表的默认字符集和字符排序规则                                 |
| Checksum        | 如果启用，保存是整个表的实时校验和                           |
| Create_options  | 创建表是指定的其他选项                                       |
| Comment         | 对于MyISAM表而言是注释信息，对于InnoDB而言是表剩余空间的信息，如果是视图会显示**VIEW** |
